...
Evidence reviewed: 2026-07-30. The authoritative brief is `MEGA_PROMPT.md`.
...
| 4. Ingestion moat | Met     | Real Serper discovery, RFC-correct robots handling, throttled bounded fetch, deterministic and mocked-LLM extraction, private snapshots, parser-versioned provenance/takedown, five general-chart sources plus four official model-level garment sources and 104 complete construction rows | Continue expanding official model-level coverage; live inventory feeds remain outside MVP scope                                         |
| 5. Matches        | Met     | Live Postgres catalog, shared scorer, filters, animated reranking, scraped and seeded candidates together, fit detail overlay, official imagery/provenance, saves, and size-tagged outbound links                                                                                           | Licensed/live inventory feeds remain outside MVP scope                                                                                  |
...
`npm run test:ingestion:garments` additionally proves four official Everlane
model sources, 104 complete point-of-measure rows, four product enrichments,
four retailer links, private snapshots, and anonymous read parity.
`npm run test:matching:live` proves one of those scraped styles ranks alongside
the seeded index, resolves the exact size, returns five construction deltas,
and produces a provenance-adjacent retailer URL.
...
| Seed corpus plus live general and model-level sources  | Met and repeatably verified                                               |
