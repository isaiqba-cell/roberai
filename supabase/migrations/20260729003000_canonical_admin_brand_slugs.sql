-- Reuse an existing canonical brand slug before deriving one from display text.
-- This avoids apostrophe variants such as Levi's -> levi-s.
...
declare
  ...
  duplicate_id uuid;
begin
  select id into canonical_id from public.brands where slug = 'levis';
  select id into duplicate_id from public.brands where slug = 'levi-s';

  if canonical_id is not null and duplicate_id is not null then
    update public.products set brand_id = canonical_id where brand_id = duplicate_id;
    update public.size_charts set brand_id = canonical_id where brand_id = duplicate_id;
    update public.styles set brand_id = canonical_id where brand_id = duplicate_id;
    ...
    update public.garment_reference_catalog
      set brand_slug = 'levis'
      where brand_slug = 'levi-s';
    delete from public.brands where id = duplicate_id;
  end if;
end;
$$;
